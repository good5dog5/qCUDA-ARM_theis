...
An example is presented in Fig. \ref{fig:fig_2_3}. This method can be implemented in $O(N^2)$ time complexity without any complex data structure.
\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.4]{image/fig_2_3}
    \caption{Calculation of rectangle union using sweep line algorithm}
    \label{fig:fig_2_3}
\end{figure}

\begin{algorithm}[H]
\label{alg:a1}
\caption{Sequence sweep line algorithm to compute rectangle union}
\DontPrintSemicolon
\renewcommand\baselinestretch{0.7}\selectfont
 \KwData{$R[N]$: a list of rectangles}
 \KwResult{$area$: the area of rectangle union}
 \SetKwProg{Fn}{Function}{}{}
 \SetKwFunction{FMain}{main}
 \SetKwProg{Pn}{Function}{:}{\KwRet}
 \Pn{\FMain}{
     $area = 0$\;
     sort all rectangles' x values\;
     sort all rectangles' y values\;
     \ForEach{$slab (x[i-1], x[i])$}{
         \eIf{$x[i-1] == x[i]$}{
             update\_active\_set($A, x[i]$)\;
         }
         {
             compute\_area($A, x[i-1], x[i]$)\;
             update\_active\_set($A. x[i]$)\;
         }
     }
 }
 \;
 \Pn{update\_active\_set($A, x$)}{
    \uIf{$x$ is a left edge of $R[k]$}{
        add $R[k]$ to active\_set $A$\;
    }
    \ElseIf{$x$ is a right edge of $R[k]$}{
        remove $R[k]$ from active\_set $A$\;
    }
 }
 \;
 \Pn{compute\_area($A, x1, x2$)}{
    $cnt = 0$\;
    \ForEach{y}{
        \If{$y$ is an edge of $R[k]$ in active\_set $A$}{
            \uIf{$y$ is a bottom edge}{
                \If{$cnt++ == 0$}{
                    $begin = y$\;
                }
            }
            \ElseIf{$y$ is a top edge}{
                \If{$--cnt == 0$}{
                    $area += (y - begin) * (x2 - x1)$\;
                }
            }
